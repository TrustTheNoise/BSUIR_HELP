...
// На плоскости задано N отрезков. Найти точку пересечения двух отрезков, имеющую 
// минимальную абсциссу. Использовать класс TreeMap.

public class Main {
	
	private static final double NO_INTERSECTION = Double.POSITIVE_INFINITY;
    ...
    private static double calculateY(LineSegment s1, LineSegment s2, double x) {
    	// y = m1 * x + (s1.y1 - m1 * s1.x1)
    	// c = y−mx = s1​.y1​−m1​*s1​.x1​
        double m1 = (s1.y2 - s1.y1) / (s1.x2 - s1.x1);
